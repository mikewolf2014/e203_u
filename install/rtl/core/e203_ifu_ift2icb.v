...
module e203_ifu_ift2icb(
  ...
  input  [`E203_PC_SIZE-1:0] ifu_req_pc, // Fetch PC, pc value for request instrcution.
...
// ===========================================================================
  ...
  // if unaligned, first request state of total two request
  localparam ICB_STATE_1ST  = 2'd1;
  // State 2: Wait to issue second request 
  // wait state between two state
  localparam ICB_STATE_WAIT2ND  = 2'd2;
  // State 3: Issued second request and wait response
  // if unaligned, second request state of total two request
  localparam ICB_STATE_2ND  = 2'd3;
  
  wire [ICB_STATE_WIDTH-1:0] icb_state_nxt;
  wire [ICB_STATE_WIDTH-1:0] icb_state_r; //current state
  wire icb_state_ena;
