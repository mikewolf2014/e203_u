...
module e203_ifu_ifetch(
  ...
  //if it is first pc value after reset from cpu_top, we can control pc
  //default vaule by assgin different value in soc level
  ...
  // handshake sucessfully, valid and ready signal high
  wire ifu_req_hsked  = (ifu_req_valid & ifu_req_ready) ;
  wire ifu_rsp_hsked  = (ifu_rsp_valid & ifu_rsp_ready) ;
  // ir to exu handshake successfully
 ...
 //    * Currently the flush can be accepted by IFU
  ...
  // The flush ack signal generation
   ...
   wire dly_flush_nxt;
      ...
      //        dly: delay, delay flush request
      //        not used, initial value should be 0
   wire dly_flush_r;
   ...
   //load reset dff, dly_flush_nxt is delayed one cycle from dly_flush_r
  ...
  // for e200 only support rv32 and rvc
  // minidec_rv32 comes from decode module, minidec_rv32 = (~(i_instr[4:2] == 3'b111)) & opcode_1_0_11;
  // 01 bit is 11, but 2-4 bits not equal 111, then it is rv32 instruction.
  wire [2:0] pc_incr_ofst = minidec_rv32 ? 3'd4 : 3'd2;

  wire [`E203_PC_SIZE-1:0] pc_nxt_pre;
  wire [`E203_PC_SIZE-1:0] pc_nxt;

  //it is bjp instruction and predict it is taken
  //prdt_taken comes from simple_bpu, minidec_bjp comes from mini_decode
  wire bjp_req = minidec_bjp & prdt_taken;

  //always 0
  wire ifetch_replay_req;
  //dly_pipe_flush_req is not used, so is 0
  wire [`E203_PC_SIZE-1:0] pc_add_op1 = 
                            ...
                            `endif//}
                               ...
                               ifu_reset_req   ? pc_rtvec :
                                                 pc_r;
  //pc incr ofst is 4 or 2, pc increment
  wire [`E203_PC_SIZE-1:0] pc_add_op2 =  
                            ...
                            `endif//}
                               ...
                               ifu_reset_req   ? `E203_PC_SIZE'b0 :
                                                 pc_incr_ofst ;
  //no flush, no reset, no bjp, so it is seq fetch instruction
