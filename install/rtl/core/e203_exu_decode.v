...
module e203_exu_decode(
  ...
  input  [`E203_INSTR_SIZE-1:0] i_instr, //32 bit instruction comes from ifu
  input  [`E203_PC_SIZE-1:0] i_pc,       //pc value of current instruction.
  input  i_prdt_taken,           //predict need to jump 
  input  i_misalgn,              // The fetch misalign, current instruction meet fetch unaligned assertion.
  input  i_buserr,               // The fetch bus error, current instruction meet fetch instruciton memory access error 
  ...
  output dec_rs1x0, // source operand 1 register index is x0
  output dec_rs2x0, // soruce operand 2 register index is x0
  output dec_rs1en, //instruction need to fetch source operand 1
  output dec_rs2en, //instruction need to fetch soruce operand 2
  output dec_rdwen, //instrctuion need to write result operand
  output [`E203_RFIDX_WIDTH-1:0] dec_rs1idx,//register index for source operand 1
  output [`E203_RFIDX_WIDTH-1:0] dec_rs2idx,//register index for source operand 2
  output [`E203_RFIDX_WIDTH-1:0] dec_rdidx, //result register index
  output [`E203_DECINFO_WIDTH-1:0] dec_info,//all other info of this instrutions is packed into a width info, it is called info bus. 
  output [`E203_XLEN-1:0] dec_imm, //immidate value of current instruction.
  output [`E203_PC_SIZE-1:0] dec_pc,
  output dec_misalgn,
  output dec_buserr,
  output dec_ilegl, //after decode, found it is illegal instruction.
  ...
  //opcode, it is 7 bit from bit0 to bit6
  ...
  //it is 32 bit instruction or 16 bit instruction
  wire rv32 = (~(i_instr[4:2] == 3'b111)) & opcode_1_0_11;

  wire [4:0]  rv32_rd     = rv32_instr[11:7]; // register dest
  wire [2:0]  rv32_func3  = rv32_instr[14:12];//func3
  wire [4:0]  rv32_rs1    = rv32_instr[19:15];//source register 1
  wire [4:0]  rv32_rs2    = rv32_instr[24:20];//source register 2
  ...
  //   * MV:     rs1= x0 (implicit),  rs2= rs2(coded),   rd = rd (coded)e203_exu.v
  ...
  assign rv_index_ilgl =
                ...
                |(| dec_rs2idx[`E203_RFIDX_WIDTH-1:3])e203_exu.v
